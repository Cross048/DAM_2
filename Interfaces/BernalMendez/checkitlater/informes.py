...
class Informes:
    ...
    def footFactura(titulo):
        try:
            ...
            var.report.drawString(440, 85, 'Subtotal: ' + var.ui.lblsubtotal.text())
            var.report.drawString(440, 70, 'IVA: ' + var.ui.lbliva.text())
            var.report.drawString(440, 55, 'Total: ' + var.ui.lbltotalfactura.text())
    ...
    def reportfactura():
        try:
            ...
            if codigofactura == '':
                ...
                var.report = canvas.Canvas('informes/' + nombre)
                conexion.Conexion.vieja(codigofactura)
                var.report.save()
                rootPath = '.\\informes'
                for file in os.listdir(rootPath):
                    if file.endswith(nombre):
