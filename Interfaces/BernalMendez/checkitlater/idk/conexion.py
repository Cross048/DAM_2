...
class Conexion():
    ...
    def vieja(codigo):
        ...
        var.report.drawString(355, 650, str(items[3]))
        var.report.drawString(415, 650, str(items[4]))
        ...
        query.bindValue(":factura", codigo)
        ...
        if query.exec():
            ...
            while query.next():
                if j <= 80:
                    ...
                    var.report.drawString(335, 650, str(items[3]))
                ...
                var.report.drawString(i + 355, j, str(query.value(4)))
                var.report.drawString(i + 405, j, str('{:.2f}'.format(round(float(query.value(4)) * float(query.value(3)), 2))) + ' €')
                j = j - 20
